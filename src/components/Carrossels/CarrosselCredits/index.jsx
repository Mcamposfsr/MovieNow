import ButtonsCarrossel from "@/components/ButtonsCarrossel"
import { useState, useRef } from "react"
import CardCredits from "@/components/Cards/CardCredits"


const CarrosselCredits = ({ cardsInfo }) => {
    

    // DEFINIR TAMANHO CARROSSEL
    const tamanho = cardsInfo.length
    ...
    return (
        <div className="h-[350px] relative w-full">
            ...
            <div className="scroll overflow-x-scroll  overflow-y-hidden flex  items-center h-full  w-full box-border box-border gap-[1.2em] px-[.5em]">
                ...
                {
                    cardsInfo.map((e, indice) => {
                        // PARA O 1 CARD FICAR GRANDE DE INICIO.

                        if (indice == foco) {
                            return <CardCredits dados={e} classNameCard="scale-105" ref={e => refCards.current[indice] = e} key={`card-${indice}`}/>

                        } else {
                            return <CardCredits dados={e} ref={e => {refCards.current[indice] = e}} key={`card-${indice}`} />
...
export default CarrosselCredits